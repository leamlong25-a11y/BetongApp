...
const Login = ({ onLoginSuccess, onSwitchToSignup }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError("អ៊ីម៉ែល ឬពាក្យសម្ងាត់មិនត្រឹមត្រូវ!");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-gray-800 p-6 sm:p-8 rounded-xl border border-gray-700 shadow-xl w-full max-w-md space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-orange-600 hover:bg-orange-500 text-white py-2.5 rounded-lg font-bold shadow-lg transition text-sm"
          >
            {loading ? "កំពុងចូល..." : "ចូលគណនី (Sign In)"}
          </button>
        </form>
